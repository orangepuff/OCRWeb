    ...
    public async Task<int> Handle(UploadPdfCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.Content.Length > maxBytes)
        {
            throw new DomainValidationException(
                $"Upload of {request.Content.Length} byte(s) exceeds the {maxBytes} byte limit.");
        }
        ...
        logger.LogInformation("{LogPrefix}: created PdfFile {FileId} ({FileName}, {Bytes} bytes) for project {ProjectId}",
            LogPrefix, file.Id, request.FileName, request.Content.Length, request.ProjectId);
