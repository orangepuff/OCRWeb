...
public class EnvironmentResolverTests
{
    ...
    {
        var resolver = new EnvironmentResolver(UnreachableConnectionString);

        var first = resolver.ResolveIdAsync("env-a", null, null);
        var second = resolver.ResolveIdAsync("env-b", null, null);
    ...
    public async Task ResolveIdAsync_AfterFailure_EvictsCacheSoNextCallRetries()
    {
        ...
        var failedTask = resolver.ResolveIdAsync("retry-env", null, null);
        await Assert.ThrowsAnyAsync<Exception>(() => failedTask);

        var retryTask = resolver.ResolveIdAsync("retry-env", null, null);
